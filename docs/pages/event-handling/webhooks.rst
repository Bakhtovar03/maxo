...
- **WebhookEngine**: Ядро, отвечающее за обработку входящих запросов. Оно получает запрос, проверяет его безопасность, парсит обновление и передает его в :class:`~maxo.dispatcher.Dispatcher`.
...
- ``on_startup``: Вызывает ``BeforeStartup`` и ``AfterStartup`` сигналы диспетчера, а также устанавливает вебхук (вызывает метод `subscribe` к API Max.ru).
